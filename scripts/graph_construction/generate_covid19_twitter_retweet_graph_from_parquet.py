"""Build a COVID-19 Twitter retweet graph directly from staged parquet files.

This mirrors the Ukraine/Russia parquet graph builder: graph events and edge
aggregation stay in DuckDB, and node features come from the bio embedding store.
"""
...
def _column_exists(columns: set[str], expr: str) -> bool:
    return expr in columns or expr.split(".", 1)[0] in columns
...
def _timestamp_expr(columns: set[str]) -> str:
    ...
    if "observed_at" in columns:
        exprs.append(f"try_cast({_quote_identifier('observed_at')} AS TIMESTAMP)")
    if "created_ts" in columns:
        exprs.append(f"try_cast({_quote_identifier('created_ts')} AS TIMESTAMP)")
    if "created_at" in columns:
        exprs.append(
            f"try_strptime(CAST({_quote_identifier('created_at')} AS VARCHAR), '%a %b %d %H:%M:%S +0000 %Y')"
        )
    if "date" in columns:
        exprs.append(f"try_strptime(CAST({_quote_identifier('date')} AS VARCHAR), '%a %b %d %H:%M:%S +0000 %Y')")
        exprs.append(f"try_cast({_quote_identifier('date')} AS TIMESTAMP)")
    if not exprs:
        return "NULL"
    if len(exprs) == 1:
        return exprs[0]
...
def _build_source_scan(conn: Any, parquet_files: list[str]) -> None:
    ...
    userid_expr = _coalesce_try_cast(
        columns,
        ["userid", "user.id", "user.id_str", "user_id"],
        "BIGINT",
    )
    rt_userid_expr = _coalesce_try_cast(
        columns,
        ["rt_userid", "retweeted_status.user.id", "retweeted_status.user.id_str", "retweeted_user_id"],
        "BIGINT",
    )
...
def main() -> None:
    ...
    try:
        ...
        # so future builds emit them by default. Never fail a build on enrichment.
        # Opt out with SKIP_BENCHMARK_TARGETS=1. See enrich_graph_targets.py.
        try:
            import os
