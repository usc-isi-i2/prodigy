"""Build a Ukraine/Russia retweet graph directly from staged parquet files.

This builder preserves the existing graph artifact contract while switching the
input pipeline to parquet and resolving node features from the bio embedding
store.
"""
...
DEFAULT_OUT = "data/data/ukr_rus_twitter/graphs/retweet_graph_parquet.pt"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a directed ukr_rus_twitter retweet graph from staged parquet input "
...
def _build_source_scan(conn: Any, parquet_files: list[str]) -> None:
    conn.execute(
        ...
        SELECT
            ...
            try_cast(rt_userid AS BIGINT) AS rt_userid,
            NULLIF(trim(CAST(rt_screen AS VARCHAR)), '') AS rt_screen,
            COALESCE(
                try_strptime(CAST(date AS VARCHAR), '%a %b %d %H:%M:%S +0000 %Y'),
