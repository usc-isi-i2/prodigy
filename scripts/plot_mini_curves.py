...
TITLES={'fr':'Node-only feature reconstruction','node':'Node-only link prediction','node_neighbors':'Node + 10-neighbor link prediction'}
...
for stage,title in TITLES.items():
    ...
    sections.append(f'<section id="{stage}"><h2>{title}</h2>{content}</section>')
...
</style></head><body><main><h1>All training curves and transfer matrices</h1><p>27 source models · 243 evaluation cells · seed 0 · nonzero features · Ukraine/COVID one-hop 500k-node minis.</p><p>Blue: training loss averaged over 100 updates. Orange: source-validation loss every 2,000 updates. ★: selected checkpoint. Every model stopped under the validation-plateau rule. Matrix rows are training sources; columns are evaluation targets; orange outlines mark same-graph cells.</p><p><b>Metrics:</b> FR uses squared cosine reconstruction error. LP curves use 1:5 positive/negative sampling; LP matrices use balanced degree-matched test pairs. Their BCE values are not directly comparable. AUC/BCE color scales are shared between the two LP matrices.</p><nav><a href="#fr">Node-only FR</a><a href="#node">Node-only LP</a><a href="#node_neighbors">Node + 10-neighbor LP</a></nav>'''+''.join(sections)+'''</main></body></html>'''
