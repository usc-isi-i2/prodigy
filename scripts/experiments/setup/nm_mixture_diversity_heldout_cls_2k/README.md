...
- `run_train_tucker.sh` optionally shards the plan across GPUs 2 and 3.
...
GPU 2 and loads the large merge once. Use `GPUS="2 3"` only after confirming host RAM
can hold two copies; GPUs other than 2 and 3 are currently off limits.

```bash
tmux new-session -d -s mixdiv2k \
  'export PATH="/home/mhchu/miniconda3/bin:$PATH"; \
   GPUS="2" bash scripts/experiments/setup/nm_mixture_diversity_heldout_cls_2k/run_train_tucker.sh \
