...
for i in "${!TARGETS[@]}"; do
  ...
  run="$(ls -dt state/mtpilot_NM_MT_task8_heldout_${heldout}_[0-9]*/ | head -n1)"
  ckpt="${run}checkpoint/state_dict_900.ckpt"
  [[ -f "${ckpt}" ]] || { echo "missing ${ckpt}" >&2; exit 1; }
  list="/tmp/mtpilot_task8_${heldout}.txt"
  echo "TASK8_EXCL_${heldout} ${ROOT}/${ckpt}" > "${list}"

  # Political leaning remains seen when either COVID or Election is held out because
  # the other graph supplies the same semantic family.
  ...
  python3 scripts/eval/eval_ckpts_all_graph_tasks_tucker.py \
    ...
    --task_embedding_seen_families "${seen_csv}" \
    > "/tmp/mtpilot_task8_eval_${heldout}.log" 2>&1 &
  pids+=("$!")
done
