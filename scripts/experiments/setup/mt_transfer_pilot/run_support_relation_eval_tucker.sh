...
for i in "${!TARGETS[@]}"; do
  ...
  run="$(ls -dt state/mtpilot_NM_MT_varway_supportrel_heldout_${heldout}_[0-9]*/ | head -n1)"
  ckpt="${run}checkpoint/state_dict_900.ckpt"
  [[ -f "${ckpt}" ]] || { echo "missing ${ckpt}" >&2; exit 1; }
  list="/tmp/mtpilot_supportrel_${heldout}.txt"
  echo "SUPPORTREL_EXCL_${heldout} ${ROOT}/${ckpt}" > "${list}"
  python3 scripts/eval/eval_ckpts_all_graph_tasks_tucker.py \
    ...
    > "/tmp/mtpilot_supportrel_eval_${heldout}.log" 2>&1 &
  pids+=("$!")
done
