...
if [[ ! -d "$RUN_ROOT/exclusions" ]]; then
  conda activate bio-embeddings-v001
  export LD_LIBRARY_PATH="${CONDA_PREFIX}/lib:${LD_LIBRARY_PATH:-}"
  python -u "$SCRIPT_DIR/build_exact_id_exclusions.py" \
    --graph "$GRAPH" --identity-db "$IDENTITY_DB" \
    --output-dir "$RUN_ROOT/exclusions" \
    > "$LOG_ROOT/build_exclusions.log" 2>&1
else
  for target in ukr_rus covid midterm; do
    [[ -f "$RUN_ROOT/exclusions/${target}.pt" ]] || {
      echo "incomplete existing exclusion directory: missing ${target}.pt" >&2
      exit 1
    }
  done
  [[ -f "$RUN_ROOT/exclusions/summary.json" ]] || {
    echo "incomplete existing exclusion directory: missing summary.json" >&2
    exit 1
  }
fi
...
for worker in 0 1; do
  CUDA_VISIBLE_DEVICES="$worker" python -u "$SCRIPT_DIR/evaluate_center_disjoint.py" \
    ...
    --exclusion-root "$RUN_ROOT/exclusions" \
...
echo "CENTER_DISJOINT_COMPLETE run_root=$RUN_ROOT"
