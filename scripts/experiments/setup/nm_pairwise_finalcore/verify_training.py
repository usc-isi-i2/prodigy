#!/usr/bin/env python3
"""Validate the complete shared-graph pair run and summarize throughput."""
...
from pair_plan import CHECKPOINT_STEP, build_models, checkpoint_path, physical_jobs
...
def verify(run_dir: Path) -> dict:
    ...
    if {row.get("prefix") for row in jobs} != set(expected_models):
        raise ValueError("manifest does not contain exactly the 36 pair prefixes")
    physical_gpus = set()
    steady_steps = 0
    steady_seconds = []
    for index, job in enumerate(physical_jobs()):
        row = next(row for row in jobs if row["prefix"] == job.model.model_id)
        expected_subset = ",".join(job.model.sources)
        checks = {
            "seed": 0,
            "neighbor_sampling_source_subset": expected_subset,
            "batch_size": 4,
        ...
        if result.get("steady_started") is not None and result.get("steady_finished") is not None:
            steady_seconds.append(
                (float(result["steady_started"]), float(result["steady_finished"]))
            )
