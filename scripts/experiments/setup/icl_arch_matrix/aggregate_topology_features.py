#!/usr/bin/env python3
"""Validate and compare degree/topology-only baselines with the trained matrix."""
...
def main() -> int:
    ...
    if len(rows) != len(BASELINES) * len(TARGETS):
        raise ValueError(f"expected eight topology rows, got {len(rows)}")
    ...
    if len(observed) != len(set(observed)) or set(observed) != expected:
        raise ValueError(f"topology grid mismatch: {observed}")
    for row in rows:
        if row["model_id"] != row["baseline"] or row["sources"] != []:
            raise ValueError(f"invalid topology identity/source row: {row}")
        if int(row["seed"]) != 0 or int(row["training_updates"]) != 0:
            raise ValueError(f"invalid seed/update accounting: {row}")
        if row["feature_view"] != FEATURE_VIEW or tuple(row["feature_names"]) != FEATURE_NAMES:
            raise ValueError(f"unexpected topology feature view: {row}")
        if row["topology_scope"] != "entire_loaded_target_graph":
            raise ValueError(f"unexpected topology scope: {row}")
        if not bool(row["topology_used"]) or bool(row["raw_features_used"]):
            raise ValueError(f"topology/raw feature contract violated: {row}")
    ...
    for target in TARGETS:
        ...
        if next(iter(fingerprints)) != reference[0]:
            raise ValueError(f"topology fingerprint differs from trained matrix on {target}")
    ...
    fields = [
        ...
        "feature_view", "feature_names", "topology_scope", "topology_used",
        "raw_features_used", "support_fit", "task", "dataset", "n_way",
        "n_shot", "n_query", "episodes", "queries", "episode_fingerprint",
        "roc_auc", "accuracy", "f1",
    ]
    with (output_root / "classification_long.csv").open(
        ...
        writer.writeheader()
        for row in rows:
            writer.writerow({**row, "sources": "", "feature_names": ",".join(row["feature_names"])})

    topology_df = pd.DataFrame(rows)
    topology_df[["baseline", "dataset", "roc_auc", "accuracy", "f1"]].to_csv(
        output_root / "target_summary.csv", index=False
    )
    ...
    for architecture in ARCHITECTURES:
        ...
        for baseline in BASELINES:
            anchors = topology_df[topology_df.baseline == baseline].set_index("dataset").roc_auc
    ...
    summary = {
        "protocol": "fixed_episode_directed_degree_topology_control",
        "seed": 0,
        "rows": len(rows),
        "training_updates": 0,
        ...
        "mean_roc_auc": {
            baseline: float(topology_df[topology_df.baseline == baseline].roc_auc.mean())
            for baseline in BASELINES
        },
