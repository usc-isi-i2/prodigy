#!/usr/bin/env python3
"""Validate and compare target-supervised references with the trained matrix."""

from __future__ import annotations

import argparse
import csv
import hashlib
import json
from pathlib import Path

import numpy as np
import pandas as pd

from scripts.experiments.setup.icl_arch_matrix.aggregate_results import ARCHITECTURES, TARGETS
from scripts.experiments.setup.icl_arch_matrix.evaluate_supervised_target import (
    BASELINES,
    DEFAULT_LR_GRID,
    TRAIN_UPDATES,
)


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--mlp", required=True)
    parser.add_argument("--graphsage", required=True)
    parser.add_argument("--trained-reference", required=True)
    parser.add_argument("--output-root", required=True)
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    paths = {
        "supervised_mlp": Path(args.mlp),
        "supervised_graphsage": Path(args.graphsage),
    }
    rows = []
    for baseline, path in paths.items():
        with path.open(encoding="utf-8") as handle:
            loaded = [json.loads(line) for line in handle if line.strip()]
        if any(row["baseline"] != baseline for row in loaded):
            raise ValueError(f"baseline mismatch in {path}")
        rows.extend(loaded)
    if len(rows) != len(BASELINES) * len(TARGETS):
        raise ValueError(f"expected eight supervised rows, got {len(rows)}")

    expected = {(baseline, target) for baseline in BASELINES for target in TARGETS}
    observed = [(row["baseline"], row["dataset"]) for row in rows]
    if len(observed) != len(set(observed)) or set(observed) != expected:
        raise ValueError(f"supervised grid mismatch: {observed}")
    for row in rows:
        if row["model_id"] != row["baseline"] or row["sources"] != []:
            raise ValueError(f"invalid supervised identity/source row: {row}")
        if int(row["seed"]) != 0 or int(row["training_updates"]) != TRAIN_UPDATES:
            raise ValueError(f"invalid seed/update accounting: {row}")
        if row["training_label_scope"] != "target_train_split":
            raise ValueError(f"invalid training label scope: {row}")
        if row["validation_selection"] != "best_of_two_fixed_lrs_at_update100":
            raise ValueError(f"invalid validation selection: {row}")
        if tuple(map(float, row["lr_grid"])) != DEFAULT_LR_GRID:
            raise ValueError(f"unexpected learning-rate grid: {row}")
        if bool(row["test_support_labels_used"]) or bool(row["query_labels_used_for_selection"]):
            raise ValueError(f"test label leakage contract violated: {row}")
        if not bool(row["raw_features_used"]):
            raise ValueError(f"supervised reference omitted raw features: {row}")
        expected_topology = row["baseline"] == "supervised_graphsage"
        if bool(row["topology_used"]) != expected_topology:
            raise ValueError(f"topology flag mismatch: {row}")
        for metric in ("roc_auc", "accuracy", "f1", "selected_val_roc_auc"):
            value = float(row[metric])
            if not np.isfinite(value) or not 0.0 <= value <= 1.0:
                raise ValueError(f"invalid {metric}: {value}")

    trained = pd.read_csv(args.trained_reference)
    if len(trained) != 372:
        raise ValueError(f"expected 372 trained rows, got {len(trained)}")
    trained_fingerprints = trained.groupby("dataset").episode_fingerprint.unique()
    for target in TARGETS:
        fingerprints = {row["episode_fingerprint"] for row in rows if row["dataset"] == target}
        reference = trained_fingerprints.loc[target]
        if len(fingerprints) != 1 or len(reference) != 1:
            raise ValueError(f"nonunique fingerprint on {target}")
        if next(iter(fingerprints)) != reference[0]:
            raise ValueError(f"supervised fingerprint differs from trained matrix on {target}")

    rows.sort(key=lambda row: (BASELINES.index(row["baseline"]), TARGETS.index(row["dataset"])))
    output_root = Path(args.output_root)
    output_root.mkdir(parents=True, exist_ok=True)
    fields = [
        "baseline", "model_id", "sources", "seed", "training_updates",
        "training_label_scope", "validation_selection", "lr_grid", "selected_lr",
        "selected_val_roc_auc", "tuning_results", "train_loss_final",
        "labeled_centers_seen", "node_split_sizes", "parameters", "raw_features_used",
        "topology_used", "test_support_labels_used", "query_labels_used_for_selection",
        "task", "dataset", "n_way", "n_shot", "n_query", "episodes", "queries",
        "episode_fingerprint", "roc_auc", "accuracy", "f1",
    ]
    with (output_root / "classification_long.csv").open(
        "w", newline="", encoding="utf-8"
    ) as handle:
        writer = csv.DictWriter(
            handle, fieldnames=fields, extrasaction="ignore", lineterminator="\n"
        )
        writer.writeheader()
        for row in rows:
            writer.writerow(
                {
                    **row,
                    "sources": "",
                    "lr_grid": json.dumps(row["lr_grid"], separators=(",", ":")),
                    "tuning_results": json.dumps(row["tuning_results"], separators=(",", ":")),
                    "node_split_sizes": json.dumps(row["node_split_sizes"], separators=(",", ":")),
                }
            )

    supervised_df = pd.DataFrame(rows)
    supervised_df[[
        "baseline", "dataset", "selected_lr", "selected_val_roc_auc",
        "roc_auc", "accuracy", "f1",
    ]].to_csv(output_root / "target_summary.csv", index=False)

    comparisons = []
    for architecture in ARCHITECTURES:
        architecture_cells = trained[trained.architecture == architecture]
        for baseline in BASELINES:
            anchors = supervised_df[supervised_df.baseline == baseline].set_index("dataset").roc_auc
            deltas = architecture_cells.apply(
                lambda row: float(row.roc_auc - anchors.loc[row.dataset]), axis=1
            )
            comparisons.append(
                {
                    "architecture": architecture,
                    "baseline": baseline,
                    "baseline_mean_roc_auc": float(anchors.mean()),
                    "update100_mean_roc_auc": float(architecture_cells.roc_auc.mean()),
                    "mean_delta": float(deltas.mean()),
                    "update100_cells_above_baseline_fraction": float((deltas > 0).mean()),
                }
            )
    comparison_df = pd.DataFrame(comparisons)
    comparison_df.to_csv(output_root / "architecture_comparison.csv", index=False)

    summary = {
        "protocol": "one_seed_target_supervised_fixed_test_episode_reference",
        "seed": 0,
        "rows": len(rows),
        "training_updates": TRAIN_UPDATES,
        "training_label_scope": "target_train_split",
        "validation_selection": "best_of_two_fixed_lrs_at_update100",
        "lr_grid": list(DEFAULT_LR_GRID),
        "test_support_labels_used": False,
        "query_labels_used_for_selection": False,
        "input_sha256": {
            baseline: hashlib.sha256(path.read_bytes()).hexdigest()
            for baseline, path in paths.items()
        },
        "targets": list(TARGETS),
        "episode_fingerprints": {
            target: next(row["episode_fingerprint"] for row in rows if row["dataset"] == target)
            for target in TARGETS
        },
        "mean_roc_auc": {
            baseline: float(supervised_df[supervised_df.baseline == baseline].roc_auc.mean())
            for baseline in BASELINES
        },
        "architecture_comparison": {
            architecture: {
                baseline: {
                    "mean_delta": float(
                        comparison_df.loc[
                            (comparison_df.architecture == architecture)
                            & (comparison_df.baseline == baseline),
                            "mean_delta",
                        ].iloc[0]
                    ),
                    "cells_above_fraction": float(
                        comparison_df.loc[
                            (comparison_df.architecture == architecture)
                            & (comparison_df.baseline == baseline),
                            "update100_cells_above_baseline_fraction",
                        ].iloc[0]
                    ),
                }
                for baseline in BASELINES
            }
            for architecture in ARCHITECTURES
        },
    }
    (output_root / "summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print(json.dumps(summary, indent=2, sort_keys=True))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
