...
from loo_plan import CHECKPOINT_STEP, SOURCES, build_models, checkpoint_path, physical_jobs
...
def test_plan_is_every_source_held_out_once_at_seed_zero():
    models = build_models()
    assert len(models) == 9
    assert [model.heldout for model in models] == list(SOURCES)
    assert all(len(model.sources) == 8 for model in models)
    assert all(model.heldout not in model.sources for model in models)
