...
RUN_DIR="${RUN_DIR:-${REPO_ROOT}/log/nm_leave_one_out_finalcore/shared_seed0_20260904}"
DRY_RUN="${DRY_RUN:-0}"
...
[[ "${#CONFIGS[@]}" == 9 ]] || {
  echo "expected 9 generated LOO configs in $CONFIG_DIR; run make_configs.py" >&2
  exit 2
}
[[ "$($PYTHON "$SCRIPT_DIR/loo_plan.py" | awk 'END {print NR-1}')" == 9 ]] || {
  echo "LOO plan must contain exactly 9 models" >&2
  exit 2
}
