...
def main():
    ...
    p.add_argument("--target", choices=("cp_hk", "ukr_rus"), default="cp_hk")
    ...
    if a.dry_run:
        print(json.dumps({"target":a.target,"model":a.model,"episodes":512,"queries":61440,"device":"cpu","threads":a.threads,"training":False,"encoding":False},indent=2)); return
    ...
    stages=pd.read_csv(a.cache/a.target/"stage_predictions_private.csv"); stages=stages[stages.model.eq(a.model)].set_index(["episode","sample"])
    ...
    for bi,item in enumerate(receipt["targets"][a.target]["batches"]):
        path=a.cache/a.target/item["file"];assert digest(path)==item["sha256"]
    ...
    report={"complete":True,"revision":subprocess.check_output(["git","rev-parse","HEAD"],cwd=ROOT,text=True).strip(),"seconds":time.time()-started,"device":"cpu","threads":a.threads,"model":a.model,"target":a.target,
