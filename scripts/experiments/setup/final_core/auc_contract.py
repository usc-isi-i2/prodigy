...
def metric_sidecar_path(
    logging_dir: str | Path, target: str, step: int, split: str = "test"
) -> Path:
    if split not in {"val", "test"}:
        raise ValueError(f"unsupported metric split {split!r}")
    return Path(logging_dir) / f"metrics_{split}_{target}_step{step}.json"


def load_metric_sidecar(
    logging_dir: str | Path, target: str, step: int, split: str = "test"
) -> dict[str, float]:
    path = metric_sidecar_path(logging_dir, target, step, split)
    if not path.is_file():
        raise FileNotFoundError(f"evaluation metric sidecar was not written: {path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    expected = {
        "accuracy": f"{split}_{target}_accuracy",
        "f1_macro": f"{split}_{target}_f1",
        "roc_auc_ovr_macro": f"{split}_{target}_roc_auc",
    }
    metrics = {}
