...
if [[ "$branch" != "codex/ogbl-collab-sage" ]]; then
  echo "expected codex/ogbl-collab-sage, found $branch" >&2
  exit 1
fi
...
run_root="/dataMeR1/phil/gfm/ogbl_collab_sage_lp/matched_v1"
...
for seed in "${seeds[@]}"; do
  ...
  if [[ -e "$out" ]]; then
    echo "refusing to overwrite existing output at $out" >&2
    exit 1
  fi
  python scripts/experiments/setup/ogbl_collab_sage_lp/run.py \
    ...
    --grad-clip-norm 1.0 \
    --wandb-mode online \
    --wandb-project ogbl-collab-sage-lp \
    --run-tag matched_v1
done
