...
run_root="/dataMeR1/phil/gfm/ogbl_collab_mlp_lp/concat_ordered_v1"
mkdir -p "$run_root/logs"
log="$run_root/logs/gpu${device}_seeds_$(IFS=_; echo "${seeds[*]}").log"
exec > >(tee -a "$log") 2>&1

echo "revision=$(git rev-parse HEAD) branch=$branch device=$device seeds=${seeds[*]} campaign=concat_ordered_v1"
for seed in "${seeds[@]}"; do
  out="$run_root/seed${seed}"
  if [[ -e "$out" ]]; then
    echo "refusing to overwrite existing ordered-concat output at $out" >&2
    exit 1
  fi
  python scripts/experiments/setup/ogbl_collab_mlp_lp/run.py \
    ...
    --run-tag concat_ordered_v1 \
    --campaign concat_ordered
done
