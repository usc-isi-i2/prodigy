#!/usr/bin/env bash
# Train target-unlabeled health-guided interleaving arms from a derived plan.
set -euo pipefail
...
[[ -f "$PLAN" ]] || { echo "missing health plan: $PLAN" >&2; exit 2; }
...
jobs=()
while IFS=$'\t' read -r model_id condition seed sources ranked agreements loo competence rank_counts segment_sources segment_steps; do
  [[ "$model_id" == model_id ]] && continue
  [[ "$condition" == health_guided ]] || { echo "unexpected plan condition" >&2; exit 2; }
  jobs+=("$model_id|$seed|$sources|$ranked|$rank_counts|$segment_sources|$segment_steps")
done < "$PLAN"
[[ ${#jobs[@]} -eq 3 ]] || { echo "expected three health-guided jobs" >&2; exit 2; }

worker() {
  local worker_index="$1" gpu="$2" index=0 item
  local model_id seed sources ranked rank_counts segment_sources segment_steps
  local prefix run_name checkpoint log member_seed source_seed
  for item in "${jobs[@]}"; do
    if (( index % ${#GPU_IDS[@]} == worker_index )); then
      IFS='|' read -r model_id seed sources ranked rank_counts segment_sources segment_steps <<< "$item"
      ...
      if [[ -f "$checkpoint" ]]; then
        ...
        echo "[gpu $gpu] START $model_id ranking=$ranked counts=$rank_counts utc=$(date -u +%FT%TZ)"
        "$PYTHON" -u experiments/run_single_experiment.py \
          ...
          --neighbor_sampling_source_schedule_seed "$source_seed" > "$log" 2>&1
...
MODEL_LIST="$LOG_ROOT/launch/health_model_list_${RUN_STAMP}.tsv"
printf 'model_id\tcheckpoint\tsources\n' > "$MODEL_LIST"
while IFS=$'\t' read -r model_id _condition _seed sources _rest; do
  [[ "$model_id" == model_id ]] && continue
...
echo "TRACE_HEALTH_STAGE2_COMPLETE model_list=$MODEL_LIST"
