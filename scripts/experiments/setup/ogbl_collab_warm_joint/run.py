"""Matched all-positive versus warm-positive joint training; validation only."""
...
def contract():
    return dict(name='warm_joint_v1',revision=j.revision(),arms=['bce','warm'],seeds=[0,1,2],steps=2000,interval=50,train_year=2017,validation_year=2018,selection='earliest maximum standalone validation Hits@50',objective='balanced sampled BCE; warm arm restricts positives to both endpoints incident to graph edges strictly before 2017; negatives unchanged',invariants='fixed100k negative panel,2048pos/1024uniform/1024mined,top2048 refresh10,Adam.001 wd.0001 clip5,496385 neural weights',advance='all3 paired selected differences positive and mean>=0.005; otherwise stop',test_access=False,prior_test_exploration_disclosed=True)


def main():
    p=argparse.ArgumentParser(); p.add_argument('--out',type=Path,required=True); p.add_argument('--source',type=Path,required=True); p.add_argument('--original',type=Path,required=True); p.add_argument('--arm',choices=['bce','warm'],default='warm'); p.add_argument('--seed',type=int,choices=range(3),default=0); p.add_argument('--device',default='cuda:1'); p.add_argument('--dry-run',action='store_true'); a=p.parse_args(); cfg=contract()
    ...
    active=np.zeros(len(x),dtype=bool); active[np.unique(tr['graph_edges'])]=True
    eligible=active[tr['pos']].all(axis=1)
    assert int(eligible.sum())==47546 and len(eligible)==119622
    # Eligibility uses historical adjacency only; labels merely supply the positive list.
    selection=np.flatnonzero(eligible)
    np.testing.assert_array_equal(np.sort(keys(tr['pos'][selection],len(x))),np.sort(keys(old['pos'],len(x))))
    if a.arm=='warm':
        ti=torch.as_tensor(selection,device=a.device)
        tp['p']=tuple(v[ti] for v in tp['p'])
    cohort=dict(total_positives=len(eligible),eligible_positives=int(eligible.sum()),
        trained_positives=len(tp['p'][0]),selection_sha256=hashlib.sha256(selection.tobytes()).hexdigest())
    j.save(out/'cohort.json',cohort)
    ...
    j.save(out/'config.json',cfg); w=wandb.init(project='ogbl-collab-compact-joint',group='warm-joint-v1',name=out.name,mode='offline',dir=str(out),config=cfg)
