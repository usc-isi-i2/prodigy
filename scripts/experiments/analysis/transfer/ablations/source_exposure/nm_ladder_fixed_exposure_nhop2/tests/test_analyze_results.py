...
from scripts.experiments.analysis.transfer.ablations.source_exposure.nm_ladder_fixed_exposure_nhop2.analyze_results import (
    assemble_logical,
...
def test_matched40k_h2_comparison_is_complete_and_protocol_matched():
    logical = assemble_logical(pd.read_csv(RAW), load_plan(MANIFEST))
    matched40k_path = (
        REPO / "scripts/experiments/analysis/transfer/ablations/context_hops/nm_ladder_nhop2"
...
def test_matched40k_h1_comparison_is_preserved_and_cross_protocol():
    logical = assemble_logical(pd.read_csv(RAW), load_plan(MANIFEST))
    matched40k_path = (
        REPO / "scripts/experiments/analysis/transfer/ladders/prodigy_nm/canonical/nm_ladder"
