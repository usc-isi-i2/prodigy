...
def plot_mean_endpoint_trajectories(long: pd.DataFrame, output_dir: Path) -> None:
    ...
    columns = [
        ...
        ("final", "matched2500", "B", "Matched 2.5k · D"),
    ...
    for family, variant, order_name, _ in columns:
        if family == "legacy":
            ...
            panels = {
                0: historical["classification"][historical["classification"]["order"] == order_name],
                1: historical["static_lp"][historical["static_lp"]["order"] == order_name],
                2: historical["nm"][(historical["nm"]["order"] == order_name) & historical["nm"]["dataset"].isin(classification_targets)],
            }
        for row in range(3):
