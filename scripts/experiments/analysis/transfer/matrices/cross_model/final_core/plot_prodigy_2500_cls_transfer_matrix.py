...
def build_matrix() -> pd.DataFrame:
    ...
    matrix = selected.groupby(["model_id", "dataset"])["roc_auc"].mean().unstack("dataset").reindex(index=MODELS, columns=TARGETS)
...
def plot(matrix: pd.DataFrame) -> None:
    ...
    ax.text(0, 1.055, "Three-seed mean ROC-AUC · available single-source checkpoints plus the all-nine mixture · summaries span five targets", transform=ax.transAxes, color="#77746e", fontsize=9)
