"""Plot the repaired feature KS results with Election excluded (no resampling)."""
import json
...
names = [n for n in r['graphs'] if n != 'election2020']
labels = ['UKR/RUS', 'COVID', 'Midterm', 'COVID Pol.', 'Suspended', 'TwiBot20', 'CP/HK', 'Facebook']
...
fig.suptitle('Mean per-coordinate feature KS · Election excluded', fontsize=16, y=.97)
fig.text(.5, .915, '10,000 nodes per graph · 768 dimensions · repaired COVID Political and Suspended', ha='center', fontsize=11)
...
for extension in ['png', 'pdf']:
    fig.savefig(ROOT / f'figures/feature_ks_matrix_without_election.{extension}', dpi=180)
