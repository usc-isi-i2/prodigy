...
The substantive 18-model pipeline launched at 2026-09-06 11:04:09 UTC in
...
The first launch at 11:01:24 UTC failed on import before creating a run directory:
prepending conda's base bin in an already-activated child selected the base Python.
The mismatch was reproduced read-only, and the retry uses the explicit
`/home/mhchu/miniconda3/envs/prodigy/bin/python`. The original failed-start log is
preserved as `readout_constraint_training_20260906_failed_environment.log` beside
the active pipeline log. No training state was overwritten or resumed.
