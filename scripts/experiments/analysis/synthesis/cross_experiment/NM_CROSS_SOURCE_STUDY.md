...
- **Build a matrix / comparison** (metrics already in the eval JSONs):
  ```bash
  python3 scripts/experiments/analysis/transfer/matrices/prodigy_nm/merged_vs_single/nm_transfer_matrix/build_auc_matrix.py \
    --log-root log --shots 3 --n-way 30 --metric all
  python3 scripts/experiments/analysis/transfer/ablations/prodigy_nm/episode_sampling/nm_cross_source_shortcut/compare_shortcut.py \
    --log-root log --shots 3 --n-way 30 --metric all
  ```
