...
- **`features_only` reuses `leakage_baseline.py --features raw`** — raw bio embeddings
  ...
  own file, `scripts/experiments/analysis/evaluation/shared_task_tables/node_regression/data/features_only_floor.csv` (23 rows),
...
- Both floor passes ran in an **isolated git worktree** off the pushed commit (the main
  ...
  `scripts/experiments/analysis/objectives/probe_matrix/pretrain_probe_matrix/plot_probe_matrix.py`.
