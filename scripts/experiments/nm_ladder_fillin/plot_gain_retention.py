...
One-panel figure for the NM interpolation ladder, making two points at a glance:
...
coral arrow is the graph added at that rung, drawn before (hollow, OOD) -> after
(filled, in-training).
...
def main() -> None:
    ...
    # (2) incumbent mean at rung r = mean AUC over the r in-training columns (0..r-1)
    incumbent = [sum(M[r][:r]) / r for r in rungs]
    # (1) newcomer at rung r: graph added is col (r-1); before = rung r-1 model on it
    #     (still OOD), after = rung r model on it (now in-training)
    newcomer = {r: (M[r - 1][r - 1], M[r][r - 1]) for r in range(2, 9)}

    blue, coral, dcoral = "#185FA5", "#D85A30", "#993C1D"
    fig, ax = plt.subplots(figsize=(8.4, 5.0))

    # (2) retention band + line
    ax.axhspan(min(incumbent), max(incumbent), color=blue, alpha=0.07, zorder=0)
    ax.plot(rungs, incumbent, "-o", color=blue, lw=2.3, ms=6, zorder=3)

    # (1) newcomer jumps
    for r, (b, a) in newcomer.items():
        ax.annotate("", xy=(r, a), xytext=(r, b),
                    arrowprops=dict(arrowstyle="-|>", color=coral, lw=2.2,
                                    shrinkA=0, shrinkB=0), zorder=4)
        ax.plot(r, b, "o", mfc="white", mec=coral, mew=1.6, ms=7, zorder=5)
        ax.plot(r, a, "o", color=coral, ms=7, zorder=5)
        ax.annotate(f"{SHORT[r - 1]}\n+{a - b:.2f}", xy=(r, a), xytext=(r + 0.14, a),
    ...
    handles = [
        Line2D([], [], color=blue, lw=2.3, marker="o", ms=6,
               label="existing graphs (mean, in-training)"),
        ...
        Line2D([], [], color=coral, lw=0, marker="o", mfc="white", mec=coral, mew=1.6,
               ms=7, label="…and before it was added (out-of-dist)"),
    ]
    ax.legend(handles=handles, loc="lower right", fontsize=9, framealpha=0.92)

    fig.tight_layout()
    out = Path(__file__).resolve().parent / "nm_ladder_gain_retention"
