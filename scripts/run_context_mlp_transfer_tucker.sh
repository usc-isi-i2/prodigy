...
for worker in 0 1 2 3; do PYTHONPATH=src python -m mixture_scaling.context_mlp_transfer --config configs/context_mlp_transfer.yaml --view "$VIEW" --objective "$OBJECTIVE" --selection "$SELECTION" --worker-index "$worker" --workers 4 --device "$worker" --output-root "$STATE_ROOT" --cache-root "$STATE_ROOT/_cache" --seed 0 "${FANOUT_ARGS[@]}" >"$LOG_ROOT/worker_${worker}.log" 2>&1 & pids[$worker]=$!; done
