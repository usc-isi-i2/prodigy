#!/bin/bash
# Step 3: Evaluate (midterm LP -> covid LP) checkpoint on ukr_rus_twitter (NM + LP + PL, 1,5,10-shot).
...
cat > "$MODEL_LIST" <<EOF
# Exp4: midterm LP -> covid LP -> eval ukr_rus (NM + LP + PL)
midterm_lp_to_covid_lp ${CKPT}
EOF
...
echo "Submitting [Exp4] ukr_rus eval (NM + LP + PL, 1,5,10-shot)..."
