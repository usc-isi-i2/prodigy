#!/bin/bash
# Step 3: Evaluate (midterm LP -> ukr_rus LP) checkpoint on covid19_twitter (NM + LP + PL, 1,5,10-shot).
...
cat > "$MODEL_LIST" <<EOF
# Exp5: midterm LP -> ukr_rus LP -> eval covid19_twitter (NM + LP + PL)
...
echo "Submitting [Exp5] covid eval (PL, 1,5,10-shot)..."
