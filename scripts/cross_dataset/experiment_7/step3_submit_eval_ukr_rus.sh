#!/bin/bash
# Step 3: Evaluate (midterm NM -> covid LP) checkpoint on ukr_rus_twitter (NM + LP + PL, 1,5,10-shot).
...
cat > "$MODEL_LIST" <<EOF
# Exp7: midterm NM -> covid LP -> eval ukr_rus (NM + LP + PL)
midterm_nm_to_covid_lp ${CKPT}
EOF
...
echo "Submitting [Exp7] ukr_rus eval (NM + LP + PL, shots=1,5,10)..."
