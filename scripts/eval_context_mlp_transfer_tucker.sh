...
for worker in 0 1 2 3; do PYTHONPATH=src python -m mixture_scaling.evaluate_context_mlp --config configs/node_only_transfer.yaml --view "$VIEW" --objective "$OBJECTIVE" --worker-index "$worker" --workers 4 --device "$worker" --state-root "$STATE_ROOT" --output-root "$RESULTS_ROOT" --seed 0 >"$LOG_ROOT/worker_${worker}.log" 2>&1 & pids[$worker]=$!; done
