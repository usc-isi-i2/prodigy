...
for stage,title in TITLES.items():
    ...
    else:content+='<div class="matrices">'+img(stage+'_uniform_auc','Uniform 1:5 transfer matrix: raw-dot AUC ↑')+img(stage+'_uniform_bce','Uniform 1:5 transfer matrix: raw-dot BCE ↓')+'</div>'
    if stage=='node_neighbors_disjoint':
        with (ROOT/'disjoint_comparison.csv').open() as handle:
            rows=list(csv.DictReader(handle))
        table='<h3>Training gaps: original vs disjoint context</h3><p>Gaps are validation BCE minus training BCE at each run’s selected checkpoint. Training is unchanged by the evaluation update.</p><div style="overflow:auto"><table><thead><tr><th>Source</th><th>Old gap</th><th>New gap</th></tr></thead><tbody>'
        for row in rows:
            table+='<tr><td>'+row['label']+'</td>'+''.join('<td>'+format(float(row[k]),'.3f')+'</td>' for k in ['old_gap','new_gap'])+'</tr>'
...
</style></head><body><main><h1>All training curves and transfer matrices</h1><p>27 source models · 243 evaluation cells · seed 0 · nonzero features · Ukraine/COVID one-hop 500k-node minis.</p><p>Blue: training loss averaged over 100 updates. Orange: source-validation loss every 2,000 updates. ★: selected checkpoint. Checkpoint selection uses source validation; final and selected updates appear on each panel. Matrix rows are training sources; columns are evaluation targets; orange outlines mark same-graph cells.</p><p><b>Metrics:</b> FR uses squared cosine reconstruction error. LP curves use 1:5 positive/negative sampling; LP matrices now also use uniform independent-endpoint negatives at 1:5, rejecting all known edges and self-loops. Raw BCE is on the same class ratio as the curves; constant-p=1/6 baseline BCE is 0.4506. A separate balanced-test BCE is retained in the CSVs. AUC/BCE color scales are shared between the two LP matrices.</p><p><b>Corrected neighbor protocol:</b> The original 70% training-edge pool is split equally into context-only edges and positive supervision edges. No positive training pair appears in neighbor context. Positive validation/test edges are unchanged. Negative evaluation pairs have been replaced with uniform samples. Mean selected-checkpoint train–validation gap: <b>0.0315 → 0.0051 (84% smaller)</b>. <a href="gallery_original.html">Original overlapping-context gallery</a> · <a href="gallery_degree_matched.html">Previous degree-matched evaluation</a>.</p><nav><a href="#fr">Node-only FR</a><a href="#node">Node-only LP</a><a href="#node_neighbors">Node + 10-neighbor LP</a></nav>'''+''.join(sections)+'''</main></body></html>'''
