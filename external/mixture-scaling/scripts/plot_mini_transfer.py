...
def plot(path,metric,title,name):
 ...
 fig,ax=plt.subplots(figsize=(11.8,8.4));limits={'vmin':.5,'vmax':.9} if metric=='auc' else ({'vmin':.65,'vmax':3.0} if metric=='bce' else {})
 if metric=='bce' and '_uniform' in name:
  values=pd.concat([pd.read_csv(ROOT/v/'matrix.csv') for v in ['node_uniform','node_neighbors_disjoint_uniform']])['bce']
  limits={'vmin':min(.45,float(values.min())),'vmax':max(.70,float(values.max()))}
 im=ax.imshow(a,cmap='viridis_r' if metric!='auc' else 'viridis',**limits)
...
if __name__=='__main__':
 ...
 if (ROOT/'node_uniform/matrix.csv').exists():
  for stage,label in [('node_uniform','Node-only LP'),('node_neighbors_disjoint_uniform','Node + 10-neighbor LP · disjoint context')]:
   for metric in ['auc','bce']:plot(ROOT/stage/'matrix.csv',metric,label+' • uniform 1:5 • raw-dot '+metric.upper()+(' ↑' if metric=='auc' else ' ↓'),stage+'_'+metric)
