...
def metric_switch(stage):
    controls=f'<div class="metric-switch" role="group" aria-label="Transfer matrix metric"><input type="radio" name="{stage}-metric" id="{stage}-auc" value="auc" checked><label for="{stage}-auc">AUC ↑</label><input type="radio" name="{stage}-metric" id="{stage}-bce" value="bce"><label for="{stage}-bce">BCE ↓</label><div class="metric-panels">'
    return controls+'<div data-metric="auc">'+img(stage+'_auc','Transfer AUC ↑ · higher is better')+'</div><div data-metric="bce">'+img(stage+'_bce','Transfer BCE ↓ · lower is better · constant-p baseline: 0.4506')+'</div></div></div>'
...
def main():
    ...
    for stage,title,old in zip(STAGES,TITLES,['node_uniform','node_neighbors_disjoint_uniform']):
        ...
        sections.append(f'<section id="{anchor}"><h2>{title}</h2><p>{note}</p>'+img(stage+'_curves','Training and source-validation curves — nine models')+metric_switch(stage)+'</section>')
