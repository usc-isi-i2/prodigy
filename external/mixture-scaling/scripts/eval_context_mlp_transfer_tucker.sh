...
for gpu in "${devices[@]}"; do [[ "$gpu" =~ ^[0-3]$ ]] || { echo "invalid GPU $gpu" >&2; exit 2; }; done
for worker in "${!devices[@]}"; do gpu="${devices[$worker]}"; PYTHONPATH=src python -m mixture_scaling.evaluate_context_mlp --config configs/context_mlp_transfer.yaml --view "$VIEW" --objective "$OBJECTIVE" --worker-index "$worker" --workers "$workers" --device "$gpu" --state-root "$STATE_ROOT" --output-root "$RESULTS_ROOT" --seed 0 "${FANOUT_ARGS[@]}" >"$LOG_ROOT/worker_${worker}.log" 2>&1 & pids[$worker]=$!; done
