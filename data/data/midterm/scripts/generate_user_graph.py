...
def parse_args():
    p = argparse.ArgumentParser(
        description="Build unified midterm graph with retweet or interaction edges, temporal views, and optional embeddings."
    )
    ...
    p.add_argument("--graph_mode", choices=["retweet", "interaction"], default="retweet")
...
def load_raw_rows(
    ...
    usecols = {
        ...
        "reply_userid", "reply_screen", "qtd_userid", "qtd_screen", "mentionid",
            ...
            if max_nodes > 0:
                try:
                    if graph_mode == "retweet":
                        file_events = prepare_retweet_rows(chunk, strict_dates)
                    else:
                        file_events = prepare_interaction_rows(chunk, strict_dates)
                except RuntimeError:
...
def normalize_ids_and_timestamps(df: pd.DataFrame, strict_dates: bool) -> pd.DataFrame:
    ...
    df["reply_userid"] = pd.to_numeric(df.get("reply_userid"), errors="coerce")
    df["qtd_userid"] = pd.to_numeric(df.get("qtd_userid"), errors="coerce")
...
def _parse_int_list_field(val) -> List[int]:
    out: List[int] = []
    for item in _parse_list_field(val):
        try:
            out.append(int(item))
        except Exception:
            continue
...
def prepare_retweet_rows(df: pd.DataFrame, strict_dates: bool) -> pd.DataFrame:
    ...
    rt["target_userid"] = rt["rt_userid"].astype(np.int64)
    rt["interaction_type"] = "retweet"
    rt["target_screen"] = None
    rt = rt[rt["userid"] != rt["target_userid"]].copy()
    if rt.empty:
        raise RuntimeError("No valid retweet rows after cleaning")
    return rt[common_cols + ["target_userid", "target_screen", "interaction_type"]]
...
def prepare_interaction_rows(df: pd.DataFrame, strict_dates: bool) -> pd.DataFrame:
    ...
    frames = []
    try:
        frames.append(prepare_retweet_rows(df, strict_dates=False))
    except RuntimeError:
        pass

    reply = pd.DataFrame()
    if "reply_userid" in df.columns:
    ...
    if not reply.empty:
        ...
        reply["target_screen"] = reply.get("reply_screen", None)
        reply["interaction_type"] = "reply"
        reply = reply[reply["userid"] != reply["target_userid"]].copy()
        if not reply.empty:
            frames.append(reply[common_cols + ["target_userid", "target_screen", "interaction_type"]])

    quote = pd.DataFrame()
    if "qtd_userid" in df.columns:
    ...
    if not quote.empty:
        ...
        quote["target_screen"] = quote.get("qtd_screen", None)
        quote["interaction_type"] = "quote"
        quote = quote[quote["userid"] != quote["target_userid"]].copy()
        if not quote.empty:
...
def trim_events_to_max_nodes(events: pd.DataFrame, max_nodes: int, graph_mode: str) -> pd.DataFrame:
    ...
    rt2 = events.loc[keep_rows].copy()
    actual_nodes = sorted(set(rt2["userid"].tolist()) | set(rt2["target_userid"].tolist()))
    print(
...
def aggregate_retweet_edge_features(rt: pd.DataFrame) -> pd.DataFrame:
    edge_grp = rt.groupby(["userid", "target_userid"], as_index=False).agg(
...
def aggregate_interaction_edge_features(events: pd.DataFrame) -> pd.DataFrame:
    edge_grp = events.groupby(["userid", "target_userid"], as_index=False).agg(
        first_ts=("timestamp", "min"),
    )
    type_counts = (
        events.groupby(["userid", "target_userid", "interaction_type"])
        .size()
    ...
    for interaction_type, feature_name in {
        "retweet": "n_retweets",
        "quote": "n_quotes",
        "reply": "n_replies",
        "mention": "n_mentions",
    }.items():
        counts = edge_grp[interaction_type] if interaction_type in edge_grp.columns else pd.Series(0.0, index=edge_grp.index)
...
def to_edge_tensors(edge_df: pd.DataFrame, id_to_idx: Dict[int, int], edge_feature_names: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    tmp["dst"] = tmp["target_userid"].map(id_to_idx)
...
def build_node_features(
    ...
    base = base_df.dropna(subset=["userid"]).copy()
...
def main():
    ...
    if args.graph_mode == "retweet":
        ...
        aggregate_edge_features = aggregate_retweet_edge_features
        base_for_features = events
        default_view_name = "retweet_all"
    else:
        events = prepare_interaction_rows(raw, args.strict_dates)
        edge_feature_names = ["first_interaction_time", "n_retweets", "n_quotes", "n_replies", "n_mentions"]
        aggregate_edge_features = aggregate_interaction_edge_features
        base_for_features = raw
    ...
    user_ids = np.array(sorted(set(events["userid"].tolist()) | set(events["target_userid"].tolist())), dtype=np.int64)
    u2i = {int(uid): i for i, uid in enumerate(user_ids)}
    print(f"Nodes: {len(user_ids):,}")

    edge_all_df = aggregate_edge_features(events)
    ...
    labeled_before_emb = int((y >= 0).sum().item())
    print(f"Node features: {x.shape[1]} dims before embeddings")
    print(f"Weak labels: {labeled_before_emb:,} / {len(user_ids):,} labeled")
    ...
    if not args.no_temporal_views:
        ...
        hist_rt = events_sorted.iloc[:cutoff_idx].copy()
        fut_rt = events_sorted.iloc[cutoff_idx:].copy()
        ...
        hist_pairs = set(zip(hist_edges_df["userid"].astype(int), hist_edges_df["target_userid"].astype(int)))
        fut_pairs = set(zip(fut_edges_df["userid"].astype(int), fut_edges_df["target_userid"].astype(int)))
