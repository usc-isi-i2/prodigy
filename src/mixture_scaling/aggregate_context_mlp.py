...
def main():
    ...
    state=Path(a.state_root); results=Path(a.results_root); targets=SOURCE_ORDER if a.objective=="fp" else LP_TARGETS; rows=[]; missing=[]
    for run_id,(source,) in singleton_rows():
        ...
        for target in targets:
            ...
            row={"view":a.view,"objective":a.objective,"source":source,"target":target,"checkpoint_step":value["checkpoint_step"]}
    ...
    receipt={"status":"complete","view":a.view,"objective":a.objective,"training_models":9,"evaluation_cells":len(rows)}; (out/f"{a.view}_{a.objective}_COMPLETE.json").write_text(json.dumps(receipt,indent=2)+"\n"); print(json.dumps(receipt)); return 0
