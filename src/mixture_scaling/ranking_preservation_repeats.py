"""Three continuation-seed repeats; fixed seed-0 checkpoints and graph/eval splits."""
...
def rows():
    return [(f'warm_{a}__and__{b}__rank{weight}__cont{seed}',a,b,weight,seed) for seed in (1,2,3) for a,b in CASES for weight in (0,1)]
...
def main():
    ...
    p.add_argument('--wandb-mode',default='offline');p.add_argument('--wandb-project',default='nonzero-mini-transfer');p.add_argument('--wandb-group',default='ranking-preservation-repeats')
    ...
    if args.seed!=0:p.error('these repeats use fixed seed-0 checkpoints and graph/validation splits')
    root=Path(args.root)
    if args.phase=='plan':
        print(json.dumps(dict(inputs=base.preflight(load_config(args.config)),runs=[dict(run_id=r,source_A=a,source_B=b,weight=w,continuation_seed=seed,checkpoint=identity(SOURCE_ROOT/f'ss_{a}'/'best.pt')) for r,a,b,w,seed in rows()]),indent=2));return
    if args.phase=='aggregate':
        results=[]
        for run_id,a,b,w,seed in rows():
            for target in base.SOURCES:
                d=json.loads((root/'results/node_neighbors_bias'/f'{run_id}__to__{target}.json').read_text())
                results.append(dict(first=a,second=b,rank_weight=w,continuation_seed=seed,target=target,checkpoint_step=d['checkpoint_step'],**d['metrics']['test']))
        ...
        atomic_json(root/'results/COMPLETE.json',dict(status='complete',models=12,evaluation_cells=len(results)));return
    ...
    if args.phase=='eval':
        args.views=('node_neighbors',);args.targets=base.SOURCES;args.model_rows=[(r,f'{a}+{b}:rank{w}:cont{seed}') for r,a,b,w,seed in rows()];base.evaluate(args,device);return
    ...
    for run_id,a,b,w,seed in rows():
        with (claims/(run_id+'.lock')).open('a') as lock:
            ...
            args.warm_start=str(SOURCE_ROOT/f'ss_{a}'/'best.pt');args.rank_weight=w;args.continuation_seed=seed
