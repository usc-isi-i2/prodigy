...
def main():
    ...
    for path in files:
        ...
        if data.get("status") != "complete" or data.get("budgets") != [1, 5, 10, 20, 50]: raise ValueError(path)
        ...
        for result in data["results"]:
            rows.append({"target": data["target"], "condition": data["condition"], "objective": data["objective"],
                         ...
                         "labels_per_class": result["labels_per_class"], "auc": result["roc_auc_ovr_macro"],
    ...
    audit = {"status": "complete", "evaluation_cells": 72, "metric_rows": len(rows), "budgets": [1, 5, 10, 20, 50]}
